...
// Recognizes a data line like "pcs 40 70 0 08/11/2025 8:41 pm" — optionally
// with the product description inline before it on the same line, e.g.
// "99J LONG AVIS pcs 57 80 0 02/07/2026 8:06 am". PDF text extraction often
// keeps a whole short row on one line, but wraps longer product names onto
// their own line(s) before the data — this pattern has to handle both.
const DATA_LINE = /^(.*?)\s*\b([A-Za-z]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+(\d{1,2}\/\d{1,2}\/\d{4})\s+\d{1,2}:\d{2}\s*(?:am|pm)?\s*$/i

// Repeating page headers and report metadata — recognized by their
// structural signature (column-header keywords, or "Key : value" report
// info lines) rather than any specific business name, so this works
// regardless of what business generated the PDF.
function isNoiseLine(line: string): boolean {
  ...
  if (/^created on\s*:/i.test(line)) return true
  if (/^generated by\s*:/i.test(line)) return true
  return false
}
